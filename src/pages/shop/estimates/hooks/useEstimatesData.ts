...
import { Estimate, EstimateStatus } from '../types/estimateTypes';
...
export function useEstimatesData() {
  ...
  const createEstimate = async (formValues: EstimateFormValues) => {
    try {
      console.log("useEstimatesData - createEstimate called with:", formValues);
      ...
      console.log("useEstimatesData - Inserting new estimate:", newEstimate);
      ...
      if (insertError) {
        console.error("useEstimatesData - Insert error:", insertError);
        throw insertError;
      }
      
      console.log("useEstimatesData - Insert successful, data:", data);
      ...
      if (formValues.line_items && formValues.line_items.length > 0 && data && data.length > 0) {
        ...
        console.log("useEstimatesData - Inserting line items:", lineItems);
        ...
        if (lineItemError) {
          console.error("useEstimatesData - Line item insert error:", lineItemError);
      ...
      console.error("useEstimatesData - Create estimate error:", error);
      toast({
        variant: "destructive",
        title: "Error",
